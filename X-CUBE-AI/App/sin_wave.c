...
#define AI_NET_OBJ_INSTANCE g_sin_wave
 
#undef AI_SIN_WAVE_MODEL_SIGNATURE
#define AI_SIN_WAVE_MODEL_SIGNATURE     "0xe6c310365055be2ff9dbc096a2b21b2b"
...
#define AI_TOOLS_DATE_TIME   "Wed May 15 02:08:17 2024"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_SIN_WAVE_N_BATCHES
#define AI_SIN_WAVE_N_BATCHES         (1)

static ai_ptr g_sin_wave_activations_map[1] = AI_C_ARRAY_INIT;
static ai_ptr g_sin_wave_weights_map[1] = AI_C_ARRAY_INIT;
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    ...
    128, NULL, NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_SIN_WAVE_IN_NUM, &serving_default_dense_input0_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_SIN_WAVE_OUT_NUM, &gemm_2_output),
  &gemm_0_layer, 0xbdee2f7f, NULL)
  ...
  ai_network* net_ctx, const ai_network_params* params)
{
  ...
  if (ai_platform_get_activations_map(g_sin_wave_activations_map, 1, params)) {
    ...
    serving_default_dense_input0_output_array.data = AI_PTR(g_sin_wave_activations_map[0] + 60);
    serving_default_dense_input0_output_array.data_start = AI_PTR(g_sin_wave_activations_map[0] + 60);
  ...
  ai_network* net_ctx, const ai_network_params* params)
{
  ...
  if (ai_platform_get_weights_map(g_sin_wave_weights_map, 1, params)) {
    ...
    gemm_1_weights_array.data = AI_PTR(g_sin_wave_weights_map[0] + 128);
    gemm_1_weights_array.data_start = AI_PTR(g_sin_wave_weights_map[0] + 128);
    gemm_1_bias_array.format |= AI_FMT_FLAG_CONST;
    gemm_1_bias_array.data = AI_PTR(g_sin_wave_weights_map[0] + 1152);
    gemm_1_bias_array.data_start = AI_PTR(g_sin_wave_weights_map[0] + 1152);
    gemm_2_weights_array.format |= AI_FMT_FLAG_CONST;
    gemm_2_weights_array.data = AI_PTR(g_sin_wave_weights_map[0] + 1216);
    gemm_2_weights_array.data_start = AI_PTR(g_sin_wave_weights_map[0] + 1216);
    gemm_2_bias_array.format |= AI_FMT_FLAG_CONST;
    gemm_2_bias_array.data = AI_PTR(g_sin_wave_weights_map[0] + 1280);
    gemm_2_bias_array.data_start = AI_PTR(g_sin_wave_weights_map[0] + 1280);
    return true;
  }
  ...
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_SIN_WAVE_MODEL_NAME,
...
AI_API_ENTRY
ai_i32 ai_sin_wave_forward(ai_handle network, const ai_buffer* input)
{
